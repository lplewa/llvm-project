...
public:
  ...
  void launchKernel(DeviceKernel<KernelSignature> Kernel, uint32_t NumGroups,
    ...
    if constexpr (sizeof...(Args) == 0) {
      launchKernelImpl(Kernel.Handle, NumGroups, GroupSize, nullptr, nullptr);
    } else {
      static_assert(
      ...
      std::apply(
          [&](auto &...A) {
            ...
            launchKernelImpl(Kernel.Handle, NumGroups, GroupSize, ArgPtrs,
                             ArgSizes);
          },
          ArgsTuple);
    }
...
private:
  ...
  void launchKernelImpl(ol_symbol_handle_t KernelHandle, uint32_t NumGroups,
                        uint32_t GroupSize, void **ArgPtrs,
